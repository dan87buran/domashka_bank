...
def test_sort_by_date(transactions):
    ...
    sorted_desc = sort_by_date(transactions, descending=False)
    assert sorted_desc[1]["date"] > sorted_desc[0]["date"]

    # Сортировка по возрастанию
    sorted_asc = sort_by_date(transactions, descending=True)
    assert sorted_asc[1]["date"] < sorted_asc[0]["date"]
